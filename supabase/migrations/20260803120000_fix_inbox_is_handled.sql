-- Corrige get_cs_inbox_with_metrics (overload de 14 params usada pelo inbox
-- comercial). A versão viva (definida em 20260709_inbox_automotive_qualification.sql)
-- cravava is_handled = false e ignorava p_hide_handled. Resultado: finalizar
-- atendimento (que grava em cs_conversation_handled) nunca refletia na lista, e a
-- aba "Concluídas" ficava sempre vazia.
--
-- Este arquivo reaplica EXATAMENTE a mesma definição (35 colunas, incluindo os
-- campos de qualificação automotiva) e só muda duas coisas:
--   1) LEFT JOIN em cs_conversation_handled para derivar is_handled/handled_at/
--      handled_reason de verdade (padrão da versão 000);
--   2) passa a honrar p_hide_handled no filtro.
-- Precisa de DROP antes porque a assinatura de retorno não muda, mas mantemos o
-- DROP+CREATE idempotente (IF EXISTS) igual à migration de origem.
...
BEGIN
  ...
  WITH conversations AS (
    SELECT DISTINCT ON (CASE WHEN m.group_id IS NOT NULL THEN m.group_id::TEXT ELSE m.lead_id::TEXT END)
      ...
      -- FIX: derivar do cs_conversation_handled em vez de cravar false
  ...
  filtered AS (
    ...
    WHERE (p_sla_filter IS NULL OR
      ...
      -- FIX: honrar p_hide_handled (antes era ignorado)
