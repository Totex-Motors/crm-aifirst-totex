...
import { useOpenClawChat, OpenClawMessage, OpenClawSession } from '@/hooks/useOpenClawChat';
...
export function OpenClawChat({
  ...
  const statusText = connectionStatus === 'connected' ? 'Online' : connectionStatus === 'connecting' ? 'Conectando...' : 'Offline';
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                <span className={cn('h-2 w-2 rounded-full', statusColor, connectionStatus === 'connected' && 'animate-pulse')} />
                <span className="text-xs text-muted-foreground">{statusText} - {description}</span>
              </div>
            </div>
          </div>
        ...
        {activeSessionId ? (
          <>
            <ScrollArea className="flex-1 overflow-auto">
              <div className="space-y-6 max-w-3xl mx-auto px-4 sm:px-6 py-6">
                {messages.length === 0 && initialMessage && (
                  <div className="flex gap-3">
                    ...
                    <div className="bg-muted rounded-2xl px-5 py-4 max-w-[75%]">
                ...
                {messages.map((message) => (
                  <MessageBubble key={message.id} message={message} icon={icon} formatTime={formatTime} />
                ))}
                ...
                {isStreaming && streamingText && (
                  <div className="flex gap-3">
                    ...
                    <div className="bg-muted rounded-2xl px-5 py-4 max-w-[75%]">
                      ...
                      <span className="inline-block w-1.5 h-4 bg-primary/60 animate-pulse rounded-sm ml-0.5 -mb-0.5" />
                    </div>
                  </div>
                )}
            ...
            <div className="p-4 border-t bg-background">
              <div className="flex gap-2 max-w-3xl mx-auto items-end">
                ...
                <Textarea
                  ref={inputRef}
                  placeholder={isListening ? 'Ouvindo...' : isTyping ? 'Aguarde...' : 'Digite sua mensagem...'}
                  value={inputValue}
                  onChange={(e) => {
                  ...
                  className={cn(
                    'flex-1 min-h-[52px] max-h-[200px] resize-none py-3 px-4 text-sm transition-opacity',
                    isTyping && 'opacity-50',
...
const MessageBubble = memo(function MessageBubble({ message, icon, formatTime }: MessageBubbleProps) {
  const isUser = message.role === 'user';

  return (
    <div className={cn('flex gap-3', isUser && 'flex-row-reverse')}>
      <Avatar className={cn('shrink-0', isUser ? 'h-7 w-7' : 'h-8 w-8')}>
      ...
      <div className={cn(
          ...
          <div className="prose prose-sm dark:prose-invert max-w-none leading-relaxed [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
            <ReactMarkdown
              ...
              components={{
                ...
                p: ({ children }) => <p className="text-sm leading-relaxed my-2.5">{children}</p>,
                ...
                h1: ({ children }) => <h1 className="text-base font-bold border-b pb-2 mb-3 mt-5">{children}</h1>,
                h2: ({ children }) => <h2 className="text-sm font-semibold mt-5 mb-2">{children}</h2>,
                h3: ({ children }) => <h3 className="text-sm font-semibold mt-3 mb-1.5">{children}</h3>,
                blockquote: ({ children }) => <blockquote className="border-l-3 border-primary/40 pl-3 italic text-muted-foreground my-3">{children}</blockquote>,
                code: ({ children, className }) => {
                  const isInline = !className;
                  return isInline ? (
                    <code className="bg-background/60 px-1.5 py-0.5 rounded text-xs font-mono">{children}</code>
                  ) : (
                    <code className="block bg-background/60 p-3 rounded-lg text-xs font-mono overflow-x-auto my-3">{children}</code>
                  );
                },
                table: ({ children }) => (
                ...
                th: ({ children }) => <th className="bg-background/40 px-3 py-2 text-left font-semibold border-b">{children}</th>,
        ...
        <p className={cn('text-[10px] mt-2', isUser ? 'text-primary-foreground/60 text-right' : 'text-muted-foreground/70')}>
